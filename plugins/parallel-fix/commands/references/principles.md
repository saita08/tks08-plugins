...
### C-20: Re-entry is detectable within a session, and only there

An earlier revision of this workflow detected re-entry by reading a team file at a path the fellow could predict because the fellow had chosen the team's name; a later revision concluded that once the infrastructure began naming teams after the session, no detection was possible at all. The second conclusion overlooked where the footing actually lies. The team's name is derived from the session id — `session-` followed by the id's first segment, and the runtime exposes the id to every shell command as `CLAUDE_CODE_SESSION_ID` — so within one session the path `~/.claude/teams/session-{id prefix}/config.json` is predictable, and its `members` array is the authoritative list of every teammate that has joined and not yet been stood down. A prior run in this same session that ended without its stand-down — an interruption, a crash, a compaction — leaves its teammates in that array, dormant but addressable: a single message wakes one. Dispatching a fresh team beside them produces two overlapping teams, with the leftover teammates reporting into a run that never briefed them — workers no one approved for this moment, the same condition C-11 exists to prevent.

So the fellow reads the config before any dispatch. Members beyond the lead mean an earlier run's team is still standing; the fellow surfaces them to the user and asks whether to stand them down under the Step 10 procedure or to abort, and dispatches nothing until only the lead remains. What stays undetectable is re-entry across sessions: a past session's team carried a different name, its in-process teammates did not survive it, and no artifact ties a new session to the run a scheduler may be re-firing. For that class C-19's prompt discipline is the only guard, which is why this check complements C-19 rather than replacing it.

### C-21: What the fellow sends to the user is the run's results

Everything the fellow sends to the user is the run's results: what is being fixed, what changed in each file and why, the commits produced, the plan decisions and their grounds, and the points where the user's judgment is genuinely required. It is written in the user's language, with terms of art in their original or katakana form and everyday process words — 承認, never 批准. This is the whole of the fellow's output surface, and defining it positively is what keeps it clear — the user reads the fellow's messages to know the state of the work and to act on it, so a message shaped to that purpose carries the outcome and the decisions that are the user's to make.
...
### C-22: An event is processed from state, not from its arrivals

One completion by one teammate can reach the fellow more than once: as the teammate's own report, and as the runtime's task notification, which fires whenever the teammate's session goes idle and may fire again later for the same piece of work. The arrivals are duplicates of the event, not new events, and a fellow that treats each arrival as news processes the same completion twice — re-reviewing a plan it already approved, re-verifying a commit it already accepted, or replying to a report the teammate never re-sent. Each duplicate response lands on a teammate that has already moved on, and the exchange falls out of step one message at a time.

The remedy is to let the shared task list, not the fellow's memory of messages, decide what is new. Each task advances through its states — plan submitted, approved, implementing, committed, verified — by TaskUpdate at the moment the fellow makes the corresponding decision, so the state always records what has already been handled. On any arrival the fellow first reads the task's state: if the state already records the event, the arrival is a duplicate and is dropped; if not, the fellow acts and advances the state in the same turn. For the state to be consultable at all, every exchange must be attributable — a teammate's report opens with its name and file group and carries commit hashes (C-17), and the fellow's own instructions name the plan or commit they concern. The teammate's report is the authoritative channel for content; a task notification is only a prompt to look at the state.
