...
4. **Commit after each fix** with a commit message that describes what was fixed and why. Write plans, reports, and commit messages in the language the review feedback is written in; when that is not English, keep terms of art in their original form or that language's usual transliteration and use everyday process words. Why: commit messages persist in the repository's history, and a calqued term written by one teammate becomes house style for every later reader
...
13. **Never spawn a named or background agent** — if part of your investigation is worth delegating, spawn the helper synchronously and anonymously: never pass `name`, and always pass `run_in_background: false` explicitly, because the parameter defaults to background when omitted. Forward this rule verbatim into the helper's prompt. Why: agent names are session-global — a named agent joins the session's one team, whose lead is the fellow, so its messages land in the fellow's inbox as reports from a teammate the fellow never dispatched; a background agent outlives your turn and its completion notices go to the session rather than to you. Either way the coordinator faces workers it cannot attribute or stop
14. **Open every report with your name and your assigned file group** — Why: all teammates' messages arrive in one inbox, and the fellow files each report against the task it concerns before acting on it; an unattributed report forces the fellow to guess which task's state to consult, and a wrong guess desynchronizes the plan-review loop for two teammates at once
