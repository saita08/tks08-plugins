...
Everything written here — assignment, rubric, grader prompt, report — is written in the user's language, and when that language is not English, terms of art keep their original form or that language's usual transliteration and process words stay everyday. This matters doubly for the grader prompt: it is frozen at registration, so a calqued term written into it can never be corrected without discarding the subject's score history.
