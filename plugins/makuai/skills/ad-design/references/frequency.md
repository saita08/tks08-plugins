...
重要なのは、選ばなかった方を欠落として扱わないことである。出典プロダクトに capping が無いのは実装漏れではなく、ADR に記録された設計判断である。「持たないこと」を決定として文書に残さなければ、後任(将来の自分を含む)が善意で capping を追加し、思想を静かに壊す。持たない設計を選んだら、持たないこと自体を記録する。
...
どちらの思想を選んでも、一つの区切りで広告を複数連続表示してはならない。AdMob が明示的に禁止する配置であり、体験としても一回の中断が二回分の重さになる。ユーザー主導方式でも、同一の発火イベントから二枚目を出すことはしない。なお、隣接する別々の区切り(たとえばコピー完了の直後に一覧へ戻る操作)で連続することは、ユーザー主導方式では起こりうる受容である。その受容ができないなら、それは capping を選ぶ理由になる。

## 静寂期間と停止の単調合成

リワード視聴の効果は、全種類の広告を一定時間止めることに置く(出典では 6 時間。調整可能な定数)。広告が止まれば広告後のリワードオファーも止まるので、静寂は広告の静寂であると同時に誘導の静寂でもある。止めた利用者は本当に静かな時間を得る。
...
広告除去 IAP は、リワードの上位互換として位置づける。静寂期間の恒久版、毎回のリワード視聴のスキップ権である。こう位置づけると、無料の手段(視聴)と有料の手段(購入)が同じ一本の線に乗り、利用者は自分の負担感に応じて選べる。リワードで広告の止まる体験を知った利用者ほど、その恒久化に価値を感じる。これは近年の設計トレンドとも一致する。
