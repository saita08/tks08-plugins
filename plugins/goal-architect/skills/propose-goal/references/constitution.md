...
Write the condition in the language the user speaks, since the transcript it will be judged against is in that language. When that is not English, keep terms of art in their original or katakana form and use everyday process words (承認, never 批准) — the evaluator reads the condition literally, and a calqued term it cannot match against the transcript is a condition that never comes true.
