...
The waiting phase between spawn and findings is therefore read-only with respect to team and task state. Reading the report file is the primary observation, since the file is what authoritatively signals completion (C-12); `TaskGet` is also permitted because it does not write, but only as a corroborating check, never as the deciding one. Re-invoking the command itself is not permitted as a waiting mechanism, because the command body re-enters Step 2, and Step 2 writes. If the runtime causes this command to be re-entered (for example via `ScheduleWakeup` whose `prompt` was set to the command name), the re-entry must be detected at Step 1 and Step 2 must be skipped — see the re-entry guard there.
...
The same reasoning governs how the command decides that the teammate is done, and whether a re-entered run has already spawned one. The obvious signal is the task's `status`, read back with `TaskGet`. But the task store is resolved through the calling session's team context, and the command does not control whether that resolution succeeds: a teammate can correctly mark its task `completed` while the command's own `TaskGet` for the same id returns `Task not found`, because the two sessions resolved the id against different stores. A completion test that trusts `TaskGet` therefore inherits a failure mode the command cannot see coming and cannot prevent — the same shape of silent, structural defect that moving findings out of messages removed. So the authoritative signal for completion is the same artifact that carries the findings: a review report file that exists and holds a structured report means the teammate finished writing it, and the by-file comments file existing means every later step finished too, because nothing else produces those files and nothing produces them early. `TaskGet` may corroborate when it succeeds, but its failure is not evidence that work is unfinished, and it is never the sole basis for deciding whether to spawn. Files do not depend on session state the command cannot control; that is precisely why they, not the task store, are what the command trusts.
...
Re-entry is detected by observing artifacts that do not depend on this session's task-store resolution, because that resolution is exactly what cannot be trusted (C-12). Decide in this order:

1. If `notes/pr{N}-review-comments.md` exists, every step including Step 6b has finished; resume at Step 7.
2. Otherwise, if `notes/code-review-pr{N}.md` exists with a structured report, the teammate was spawned and has written the report, but Steps 4 through 6 have not completed; skip Step 2 and resume at Step 3.
3. Otherwise, check whether a team for this run already exists by reading `~/.claude/teams/{team-name}/config.json`, which is reachable independently of task-store resolution. If it exists, the teammate was already spawned but has not yet written the report; skip Step 2 to honor C-11 and resume the Step 2 waiting strategy.
4. Only if none of the above hold has the teammate not been spawned; proceed into Step 2 normally.

`TaskGet` may inform this judgment when it succeeds, but because a `Task not found` from it is indistinguishable from "no such task," it is never the sole basis for deciding whether to re-spawn — that decision rests on the files and the team config, which a re-spawn would otherwise duplicate in violation of C-11.
...
After the spawn returns, the teammate runs asynchronously. What the command waits for is not a findings message — the findings are never delivered as a message (C-12) — but `notes/code-review-pr{N}.md` appearing on disk with a structured report in it, which is what signals that the teammate has finished its part. The findings are then read from that file in Step 3, not received as an event. The natural impulse during this gap is to do something — poll, prod, schedule a check — but C-11 forbids any operation that writes to team or task state, since the teammate cannot tell such writes apart from a new instruction. Reading is fine; writing is not. Reading the file, or `TaskGet` as a corroborating check, are the read-only observations the runtime supports for this purpose.

The default and correct waiting strategy is passive: return control to the runtime and, on the next turn, check whether `notes/code-review-pr{N}.md` now exists with a structured report; once it does, proceed to Step 3. `TaskGet` may be consulted alongside this, but a `Task not found` from it does not mean the work is unfinished (C-12), so it never overrides what the file shows. `ScheduleWakeup` is the one mechanism that looks like it could help and most easily causes the very fault C-11 prevents — when the wakeup's `prompt` is the command name, firing it re-invokes the command, which re-enters Step 2 and re-spawns the teammate. If a wakeup is genuinely needed, the `prompt` must describe a continuation ("check whether `notes/code-review-pr{N}.md` exists yet and proceed to Step 3 once it does") rather than a re-invocation of this command. The same constraint rules out re-running `TeamCreate`, `TaskCreate`, or `Agent` for this review, sending the teammate a `SendMessage`, or rewriting the teammate's task via `TaskUpdate` — each is a write that the teammate observes as a fresh instruction.

### 3. Process teammate results

The teammate is done when `notes/code-review-pr{N}.md` exists and holds a structured review report rather than a placeholder or an empty file. That file is both the authoritative findings payload and the authoritative completion signal (C-12). Read it. `TaskGet` may be used to corroborate — a `completed` status agrees with the file — but a `TaskGet` that returns `Task not found` is not a reason to treat the review as unfinished or to wait longer: it reflects a task-store resolution gap in this session, not the teammate's progress. If the file is absent or still a placeholder, the teammate has not finished writing it; continue waiting per the Step 2 waiting strategy rather than proceeding. Do not rely on a message body or the Agent return value for the findings: a message can drop the body through a transport-field error that raises no error of its own, while the file does not decay. Do not read the teammate's output through any task-output mechanism, because the underlying file is the full subagent transcript and reading it overflows this command's context.
