...
このプラグイン自身がこの設定を配布することはできません。プラグインが同梱する `settings.json` は `agent` と `subagentStatusLine` のキーしか反映されず、フックからセッションの環境変数を設定することもできないためです。設定が不足している場合、`/broad-review` は縮退レビューを黙って行うのではなく、不足を報告して停止します。
