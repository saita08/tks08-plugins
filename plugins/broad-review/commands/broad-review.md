...
Completion usually needs no separate signal: the Agent call is synchronous, and when it returns with the subagent's completion note, the subagent has finished. The runtime reserves one exception — a synchronous call that runs long may be converted to a background task, and the call then returns immediately with a result announcing the conversion instead of the completion note. The conversion changes the completion signal, not the contract: the same single agent is still doing the same work, and its completion arrives as a notification naming it. The command waits for that notification without starting other work, and then proceeds exactly as if the call had just returned. What neither form of completion guarantees is that the work was done as instructed: if the report file is missing, or holds a placeholder rather than a structured report, once the agent has finished, the subagent did not deliver what the prompt asked for. That is a gap between instruction and reality, handled per C-10 — stop and report, rather than retrying the spawn or improvising a review.
...
The most complete artifact decides first: if `notes/pr{N}-review-comments.md` exists, every step through Step 6b has finished and the run resumes at Step 7. If only `notes/code-review-pr{N}.md` exists with a structured report, the review finished but the later steps did not, so Step 2 is skipped and the run resumes at Step 3. If neither file exists, check TaskList for a still-running delegation from this run before spawning anything: a synchronous Agent call normally dies with its run, but one the runtime converted to a background task (C-11) survives it. If such a task is running, wait for its completion notification instead of paying for a second review; otherwise no reusable work survives and Step 2 proceeds normally.
...
> You are one worker inside a code review that is orchestrated above you. Do exactly the task stated in this prompt; do not invoke any review skill or review command to start a review of your own. If you spawn an agent with the Agent tool, always pass `run_in_background: false` explicitly (when omitted, the parameter defaults to background) and never pass `name`, and include this block verbatim in that agent's prompt.
...
When the Agent call returns — or, if the runtime converted it to a background task, when the completion notification naming that agent arrives (C-11) — the review is done. The findings are read from `notes/code-review-pr{N}.md`, which is the authoritative payload (C-11). Read it. Do not take the findings from the Agent return value — it carries only a completion note — and do not read the subagent's output through any task-output mechanism, because the underlying file is the full subagent transcript and reading it overflows this command's context. If the file is missing or holds a placeholder, handle it as a delegation failure per Step 2.
